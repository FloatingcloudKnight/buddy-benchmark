...
intptr_t sizesInput2[3] = {_SIZE_K, _SIZE_BATCH, _SIZE_N};
intptr_t sizesOutput[3] = {_SIZE_M, _SIZE_BATCH, _SIZE_N};
...
void DL_OPS_MATMUL_DOUBLE_TRANSPOSE_B(benchmark::State &state, Func func) {
...
void MLIRVerification(float *outputExpected, MLIRFunctionType MLIRFunc,
  ...
  matmul_double_transpose_b::verify<float>(outputExpected, outputOptimized,
                                          _SIZE_M, _SIZE_BATCH * _SIZE_N, name);
}
...
extern "C" {
void _mlir_ciface_matmul_double_transpose_b_scalar_O0(MemRef<float, 3> *A,
                                                     MemRef<float, 3> *B,
                                                     MemRef<float, 3> *C);
void _mlir_ciface_matmul_double_transpose_b_scalar_O3(MemRef<float, 3> *A,
                                                     MemRef<float, 3> *B,
                                                     MemRef<float, 3> *C);
void _mlir_ciface_matmul_double_transpose_b_vec(MemRef<float, 3> *A,
                                               MemRef<float, 3> *B,
                                               MemRef<float, 3> *C);
/// [Step 1] Add function of your new method.
}
BENCHMARK_CAPTURE(DL_OPS_MATMUL_DOUBLE_TRANSPOSE_B, Scalar_O0,
                  _mlir_ciface_matmul_double_transpose_b_scalar_O0)
    ->Unit(benchmark::kMillisecond)
    ->Iterations(_NUM_ITER);
BENCHMARK_CAPTURE(DL_OPS_MATMUL_DOUBLE_TRANSPOSE_B, Scalar_O3,
                  _mlir_ciface_matmul_double_transpose_b_scalar_O3)
    ->Unit(benchmark::kMillisecond)
    ->Iterations(_NUM_ITER);
BENCHMARK_CAPTURE(DL_OPS_MATMUL_DOUBLE_TRANSPOSE_B, Vec,
                  _mlir_ciface_matmul_double_transpose_b_vec)
    ->Unit(benchmark::kMillisecond)
    ->Iterations(_NUM_ITER);
...
int main(int argc, char **argv) {
  ...
  input1 = matmul_double_transpose_b::allocArray<float>(_SIZE_BATCH * _SIZE_N,
                                                       _SIZE_K);
  input2 = matmul_double_transpose_b::allocArray<float>(_SIZE_BATCH * _SIZE_K,
                                                       _SIZE_M);
  ...
  _mlir_ciface_matmul_double_transpose_b_scalar_O0(&input1MemRef, &input2MemRef,
                                                  &outputMemrefScalar);
  ...
  MLIRVerification(outputExpected,
                   _mlir_ciface_matmul_double_transpose_b_scalar_O3,
                   "Scalar_O3");
  MLIRVerification(outputExpected, _mlir_ciface_matmul_double_transpose_b_vec,
                   "Vec");
